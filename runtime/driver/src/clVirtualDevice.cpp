...
//#endif
	...
	else if (device_type == CL_DEVICE_TYPE_ALL)
	{
		...
		future_context_devices[0] = std::async(async_find_device_context_pairs, _cl_platforms, CL_DEVICE_TYPE_CPU);
		future_context_devices[1] = std::async(async_find_device_context_pairs, _cl_platforms, CL_DEVICE_TYPE_GPU);
	...
	for (auto& item : _cl_devices_context)
	{
		...
		_map_devices[&ctx] = clDevice(err, count_of_platform_devices, ctx, device, did);
#else
